...
public class ChatBehaviour : NetworkBehaviour
{
  ...
  [SerializeField] private StringEvent[] SpecificChatCommands;
  [SerializeField] private StringEvent GenericChatCommand;
  [SerializeField] private bool SendCommandsToChat;
  ...
  public bool ParseCommandAndInvoke(string message)
  {
    ...
    var commandEvent = SpecificChatCommands.FirstOrDefault(c => c.name.ToLowerInvariant() == splitCommand[0].ToLowerInvariant());
    if (commandEvent == null)
    {
      Debug.Log($"Raising generic chat command (emote)");
      GenericChatCommand.Raise(splitCommand[0]);
    }
    else
    {
      Debug.Log($"Raising chat command {commandEvent.name}");
      commandEvent.Raise(splitCommand.Length > 1 ? splitCommand[1] : "empty");
    }
